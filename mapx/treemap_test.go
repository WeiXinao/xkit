...
func TestNewTreeMapWithMap(t *testing.T) {
	tests := []struct {
		name       string
		m          map[int]int
		comparable xkit.Comparator[int]
		wantKey    []int
		wantVal    []int
		wantErr    error
	}{
		{
			...
			wantKey:    nil,
			wantVal:    nil,
			wantErr:    errors.New("xkit: Comparator不能为nil"),
		},
		{
			name:       "empty",
			m:          map[int]int{},
...
// goos: windows
// goarch: amd64
// pkg: github.com/gotomicro/xkit/mapx
...
// PASS
// ok      github.com/gotomicro/xkit/mapx  0.797s
func BenchmarkTreeMap(b *testing.B) {
	hashmap := NewHashMap[hashInt, int](10)
	treeMap, _ := NewTreeMap[uint64, int](xkit.ComparatorRealNumber[uint64])
